...
Reporting.prototype.handleAfterRender = function (request, response) {
  ...
  return self.reporter.documentStore.collection('reports').insert(report, request).then(function (r) {
    report._id = r._id
    ...
    return self.reporter.documentStore.collection('reports').update({_id: report._id}, {$set: {blobName: blobName}}, request).then(function () {
      if (request.headers) {
